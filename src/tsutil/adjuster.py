...
class MainFrame(ToolFrame):
    def __init__(self, parent: wx.Window | None = None, *args, **kw):
        ...
        input_file_sizer.Add(
            wx.StaticText(input_file_panel, label="調整するステッチング画像ファイル:"),
        ...
        sizer.Add(
            wx.StaticText(
                panel,
                label="ステッチング画像の任意の場所をクリックすると、拡大画像が下に表示されます。",
            ),
            flag=wx.ALIGN_CENTER,
        )
        self.input_image_thumbnail = ImageViewer(
            panel, min_size=THUMBNAIL_SIZE, enable_zoom=False
        )
        ...
        self.output_image_thumbnail = ImageViewer(
            panel, min_size=THUMBNAIL_SIZE, enable_zoom=False
        )
        ...
        self.input_image_thumbnail.Bind(
            EVT_MOUSE_CLICK_IMAGE, self.__on_mouse_click_thumbnail
        )
    ...
    def __make_setting_panel(self, parent):
        ...
        height_sizer.Add(
            wx.StaticText(
                height_panel, label="屋根Y:", style=wx.ALIGN_RIGHT | wx.ST_NO_AUTORESIZE
            ),
            flag=wx.EXPAND | wx.ALIGN_CENTER_VERTICAL,
        )
        self.y_top = wx.SpinCtrl(
            height_panel,
            id=ID_Y_TOP,
            name="y_top",
            value="0",
        ...
        height_sizer.Add(
            wx.StaticText(
                height_panel, label="足元Y:", style=wx.ALIGN_RIGHT | wx.ST_NO_AUTORESIZE
            ),
            flag=wx.EXPAND | wx.ALIGN_CENTER_VERTICAL,
        )
        self.y_bottom = wx.SpinCtrl(
            height_panel,
            id=ID_Y_BOTTOM,
            name="y_bottom",
            value="3840",
        ...
        self.position_list = wx.ListCtrl(
            setting_panel,
            id=ID_POSITION_LIST,
            name="position_list",
            size=dpi_aware_size(parent, wx.Size(*POSITION_LIST_SIZE)),
            style=wx.LC_REPORT | wx.LC_SINGLE_SEL,
        )
        self.position_list.InsertColumn(
            0, "列車の調整位置", wx.LIST_FORMAT_LEFT, width=200
        )
        ...
        option_sizer.Add(
            self.unsharp_mask_parameter, flag=wx.EXPAND | wx.LEFT, border=MARGIN
        )
        ...
        self.preview_message = wx.StaticText(
            preview_panel, label=DEFAULT_PREVIEW_MESSAGE, style=wx.ALIGN_CENTER
        )
        ...
        if row_index is None:
            ...
            for entry, position in zip(data.positions, self.positions):
                self.position_list.Append(
                    [entry.description, str(position) if position is not None else ""]
                )
        else:
            self.position_list.SetItem(
                ...
                1,
                (
                    str(self.positions[row_index])
                    if self.positions[row_index] is not None
                    else ""
                ),
            )
    ...
    def __adjust_image(self):
        ...
        if (
            ...
            or index == wx.NOT_FOUND
            or not self.positions
            or None in self.positions
        ):
        ...
        dst_height = h
        dst_widths = (
            car_widths * (y_src_bottom - y_src_top) / car_height + 0.5
        ).astype(int)
        dst_width = dst_widths.sum()
        src_widths = np.array(
            [x2 - x1 for (x1, x2) in zip(self.positions[:-1], self.positions[1:])]
        )
        if min(src_widths) <= 0:
            raise Exception(
                "調整位置のX座標は上から小→大(画像の左→右)の順で入力してください。"
            )
        ...
        if max_ratio > 1:
            ...
            img = cv2.resize(
                self.raw_image,
                (self.raw_image.shape[1], dst_height),
                interpolation=cv2.INTER_AREA,
            )
            dst_widths = (dst_widths / max_ratio + 0.5).astype(int)
            dst_width = dst_widths.sum()
        else:
            img = self.raw_image
    ...
    def __on_mouse_click_thumbnail(self, event):
        x = int(event.image_x / self.thumb_ratio + 0.5)
    ...
    def __on_set_focus(self, event):
        ...
        if self.last_focus == ID_Y_TOP:
            self.preview_message.SetLabel(
                "画像をクリックして、屋根のY座標を決定してください。"
            )
            event.Skip()
            return
        elif self.last_focus == ID_Y_BOTTOM:
            self.preview_message.SetLabel(
                "画像をクリックして、足元のY座標を決定してください。"
            )
            event.Skip()
            return
        elif self.last_focus == ID_POSITION_LIST:
    ...
    def __on_save_button_clicked(self, event):
        ...
        if not self.positions or None in self.positions:
            wx.MessageBox(
                "調整位置の座標が設定されていません。", "エラー", wx.OK | wx.ICON_ERROR
            )
            event.Skip()
            return
        ...
        with wx.FileDialog(
            ...
            try:
                ...
                self.output_image_thumbnail.set_image_zoom_position(
                    0, dpi_aware(self, THUMBNAIL_HEIGHT) // 2, 1.0
                )
