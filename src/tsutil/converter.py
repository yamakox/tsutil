...
class MainFrame(ToolFrame):
    def __init__(self, parent: wx.Window | None = None, *args, **kw):
        ...
        output_file_sizer.Add(
            wx.StaticText(output_file_panel, label="動画ファイル:"),
            flag=wx.ALIGN_CENTER_VERTICAL,
        )
    ...
    def __make_setting_panel(self, parent):
        ...
        self.ltr_button = wx.RadioButton(
            right_panel, label="左から右へスクロール", style=wx.RB_GROUP
        )
        ...
        self.loop_no_button = wx.RadioButton(
            right_panel,
            label="終端に達したら動画は終了する(ループしない)",
            style=wx.RB_GROUP,
        )
        self.loop_no_button.SetValue(True)
        right_sizer.Add(self.loop_no_button, flag=wx.ALIGN_LEFT)
        self.loop_forward_button = wx.RadioButton(
            right_panel, label="終端に達したら先頭に戻る(ループする)"
        )
        right_sizer.Add(self.loop_forward_button, flag=wx.ALIGN_LEFT)
        self.loop_reverse_button = wx.RadioButton(
            right_panel, label="終端に達したら反対方向に戻る(ループする)"
        )
        right_sizer.Add(
            self.loop_reverse_button, flag=wx.ALIGN_LEFT | wx.BOTTOM, border=MARGIN
        )
    ...
    def __make_movie(self, output_path):
        try:
            ...
            wx.MessageBox(
                "入力パラメータエラー: " + str(excep), "エラー", wx.OK | wx.ICON_ERROR
            )
    ...
    def __movie_save_worker(
        ...
        if thumb_height.value > 0:
            ...
            thumb_img = cv2.resize(
                self.raw_image, thumb_size, interpolation=cv2.INTER_AREA
            )
        h = movie_size.height - thumb_size[1]
        w = int((self.raw_image.shape[1] * h) / self.raw_image.shape[0] + 0.5)
        if thumb_img is not None:
            thumb_w = int((movie_size.width * movie_size.width) / w + 0.5)
            thumb_ox = 0
        ...
        if frame_rate.gif:
            ...
            with FrameWriter(
                str(output_path),
                size=(movie_size.width, movie_size.height),
    ...
    def __enum_frames(
        ...
        for i, x in enumerate(x_positions):
            ...
            buf = (
                ...
                else np.empty((movie_size.height, movie_size.width, 3), dtype=np.uint8)
            )
            ...
            if thumb_img is not None:
                ...
                thumb_buf[:, _x : _x + thumb_w, :] = (
                    (thumb_buf[:, _x : _x + thumb_w, :] + THUMBNAIL_HIGHLIGHT_COLOR)
                    // 2
                ).astype(np.uint8)
                buf[: thumb_size[1], :, :] = thumb_buf[
                    :, thumb_ox : thumb_ox + thumb_size[0], :
                ]
