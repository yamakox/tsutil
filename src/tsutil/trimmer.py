...
class MainFrame(ToolFrame):
    def __init__(self, parent: wx.Window | None = None, *args, **kw):
        ...
        input_file_sizer.Add(
            wx.StaticText(input_file_panel, label="トリミングする動画ファイル:"),
        ...
        sizer.Add(
            wx.StaticText(
                panel,
                label="※ffmpegによるトリミングの結果には多少の誤差が生じます。こちらで目視確認してください。",
            ),
            flag=wx.ALIGN_CENTER,
        )
        ...
        output_file_sizer.Add(
            wx.StaticText(output_file_panel, label="トリミングした動画ファイル:"),
            flag=wx.ALIGN_CENTER_VERTICAL,
        )
        ...
        self.input_video_thumbnail.Bind(
            EVT_VIDEO_RANGE_CHANGED, self.__on_video_range_changed
        )
