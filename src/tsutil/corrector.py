...
class MainFrame(ToolFrame):
    def __init__(self, parent: wx.Window | None = None, *args, **kw):
        ...
        input_file_sizer.Add(
            wx.StaticText(input_file_panel, label='連続画像のカタログファイル:'), flag=wx.ALIGN_CENTER_VERTICAL
        )
        ...
        user_correction_sizer.Add(
            self.use_deshake_correction_button, flag=wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, border=MARGIN
        )
        ...
        self.rotation = wx.SpinCtrlDouble(
            use_correction_panel, value='0.00', min=-180.0, max=180.0, inc=0.01, style=wx.SP_ARROW_KEYS | wx.ALIGN_RIGHT
        )
        ...
        user_correction_sizer.Add(
            self.use_perspective_correction_button, flag=wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, border=MARGIN
        )
    ...
    def __make_viewer_panel(self, parent):
        ...
        base_caption_sizer.Add(
            wx.StaticText(base_caption_panel, label='ブレ補正の基準画像とブレ測定枠の設定:'), flag=wx.ALIGN_CENTER
        )
        ...
        caption_sizer.Add(
            wx.StaticText(caption_panel, label='ブレ補正後のサンプル画像と歪み補正の設定:'), flag=wx.ALIGN_CENTER
        )
        caption_sizer.Add(
            wx.StaticText(caption_panel, label='画像補正後のサンプル画像と出力範囲の設定:'), flag=wx.ALIGN_CENTER
        )
    ...
    def __make_console_panel(self, main_panel):
        ...
        shaking_sizer.Add(
            wx.StaticText(
                shaking_panel, label='左の画像をドラッグしてブレ測定枠を追加します。', style=wx.ST_NO_AUTORESIZE
            ),
            flag=wx.ALIGN_CENTER_HORIZONTAL,
        )
        shaking_sizer.Add(
            wx.StaticText(
                shaking_panel, label='また、ブレ測定枠をクリックすると削除します。', style=wx.ST_NO_AUTORESIZE
            ),
            flag=wx.ALIGN_CENTER_HORIZONTAL,
        )
        ...
        correction_sizer.Add(
            wx.StaticText(
                correction_panel, label='歪み補正(射影変換)は中央の画像の赤■を動かして', style=wx.ST_NO_AUTORESIZE
            ),
            flag=wx.ALIGN_CENTER_HORIZONTAL,
        )
        correction_sizer.Add(
            wx.StaticText(correction_panel, label='水平・垂直にする四角形を設定します。', style=wx.ST_NO_AUTORESIZE),
            flag=wx.ALIGN_CENTER_HORIZONTAL,
        )
        ...
        clip_sizer.Add(
            wx.StaticText(clip_panel, label='右の補正後画像の赤■を動かして', style=wx.ST_NO_AUTORESIZE),
            flag=wx.ALIGN_CENTER_HORIZONTAL,
        )
        clip_sizer.Add(
            wx.StaticText(clip_panel, label='画像ファイルに出力する範囲を設定します。', style=wx.ST_NO_AUTORESIZE),
            flag=wx.ALIGN_CENTER_HORIZONTAL,
        )
    ...
    def __update_base_frame_pos_text(self):
        ...
        self.base_frame_button.SetLabel(
            f'ブレ補正の基準画像を選択する: フレーム位置 {self.model.base_frame_pos}/{count}'
        )

    def __update_sample_frame_pos_text(self):
        count = self.input_video_thumbnail.get_frame_count()
        self.sample_frame_button.SetLabel(
            f'補正対象のサンプル画像を選択する: フレーム位置 {self.model.sample_frame_pos}/{count}'
        )
    ...
    def __set_sample_image_viewer(self):
        ...
        frame = (
            (self.sample_frame // 256).astype(np.uint8) if self.sample_frame.dtype == np.uint16 else self.sample_frame
        )
        deshaked_frame = cv2.warpPerspective(frame, mat, (frame.shape[1], frame.shape[0]), flags=cv2.INTER_AREA)
        if self.model.use_overlay:
            base_frame = (
                (self.base_frame // 256).astype(np.uint8) if self.base_frame.dtype == np.uint16 else self.base_frame
            )
            if angle:
    ...
    def __reset_shaking_detection_selector(self, index: int | None = None):
        ...
        if self.model.extra_shaking_detection_fields is not None:
            items.extend(
                [f'ブレ測定枠パターン - {i + 1}' for i in range(len(self.model.extra_shaking_detection_fields))]
            )
    ...
    def __load_setting(self):
        ...
        try:
            with open(path, 'r') as f:
                ...
                self.use_nega_button.SetValue(self.model.use_nega)
                self.use_grid_button.SetValue(self.model.use_grid)
                self.rotation.SetValue(_g(self.model.rotation_angle))
