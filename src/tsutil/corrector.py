...
from .components.video_thumbnail import (
    VideoThumbnail,
    EVT_VIDEO_LOADED,
    EVT_VIDEO_POSITION_CHANGED,
)
from .components.image_viewer import ImageViewer
from .components.base_image_viewer import (
    BaseImageViewer,
    EVT_FIELD_ADDED,
    EVT_FIELD_DELETED,
)
from .components.deshaking_image_viewer import (
    DeshakingImageViewer,
    EVT_PERSPECTIVE_POINTS_CHANGED,
)
from .components.clip_image_viewer import ClipImageViewer, EVT_CLIP_RECT_CHANGED
from .components.drag_drop_file_picker import DragDropFilePickerCtrl
...
class MainFrame(ToolFrame):
    def __init__(self, parent: wx.Window | None = None, *args, **kw):
        ...
        input_file_sizer.Add(
            wx.StaticText(input_file_panel, label="連続画像のカタログファイル:"),
            flag=wx.ALIGN_CENTER_VERTICAL,
        )
        self.input_file_picker = DragDropFilePickerCtrl(
            input_file_panel,
        ...
        self.input_file_picker.Bind(
            wx.EVT_FILEPICKER_CHANGED, self.__on_input_file_changed
        )
        ...
        self.base_frame_button = wx.RadioButton(
            input_frame_panel, label="ブレ補正の基準画像を選択する", style=wx.RB_GROUP
        )
        self.base_frame_button.SetValue(True)
        input_frame_sizer.Add(self.base_frame_button, flag=wx.ALIGN_CENTER)
        self.sample_frame_button = wx.RadioButton(
            input_frame_panel, label="補正対象のサンプル画像を選択する"
        )
        input_frame_sizer.Add(self.sample_frame_button, flag=wx.ALIGN_CENTER)
        input_frame_panel.SetSizerAndFit(input_frame_sizer)
        input_video_sizer.Add(
            input_frame_panel, flag=wx.ALIGN_CENTER | wx.BOTTOM, border=4
        )
        ...
        self.use_deshake_correction_button = wx.CheckBox(
            use_correction_panel, label="ブレ補正を行う"
        )
        self.use_deshake_correction_button.SetValue(self.model.use_deshake_correction)
        self.use_deshake_correction_button.Bind(
            wx.EVT_CHECKBOX, self.__on_input_value_changed
        )
        user_correction_sizer.Add(
            self.use_deshake_correction_button,
            flag=wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            border=MARGIN,
        )
        self.use_overlay_button = wx.CheckBox(
            use_correction_panel, label="基準画像を重ねて表示する"
        )
        ...
        user_correction_sizer.Add(
            self.use_nega_button,
            flag=wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            border=MARGIN,
        )
        self.use_rotation_correction_button = wx.CheckBox(
            use_correction_panel, label="画像の傾き補正(角度):"
        )
        self.use_rotation_correction_button.SetValue(self.model.use_rotation_correction)
        self.use_rotation_correction_button.Bind(
            wx.EVT_CHECKBOX, self.__on_input_value_changed
        )
        user_correction_sizer.Add(
            self.use_rotation_correction_button, flag=wx.ALIGN_CENTER_VERTICAL
        )
        self.rotation = wx.SpinCtrlDouble(
            use_correction_panel,
            value="0.00",
            min=-180.0,
            max=180.0,
            inc=0.01,
        ...
        user_correction_sizer.Add(
            self.rotation, flag=wx.ALIGN_CENTER_VERTICAL | wx.RIGHT, border=MARGIN
        )
        self.use_perspective_correction_button = wx.CheckBox(
            use_correction_panel, label="歪み補正(射影変換)を行う"
        )
        self.use_perspective_correction_button.SetValue(
            self.model.use_perspective_correction
        )
        self.use_perspective_correction_button.Bind(
            wx.EVT_CHECKBOX, self.__on_input_value_changed
        )
        user_correction_sizer.Add(
            self.use_perspective_correction_button,
            flag=wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            border=MARGIN,
        )
        self.use_grid_button = wx.CheckBox(
            use_correction_panel, label="グリッドを表示する"
        )
        self.use_grid_button.SetValue(self.model.use_grid)
        self.use_grid_button.Bind(wx.EVT_CHECKBOX, self.__on_input_value_changed)
        user_correction_sizer.Add(
            self.use_grid_button,
            flag=wx.ALIGN_CENTER_VERTICAL | wx.RIGHT,
            border=MARGIN,
        )
        ...
        save_button = wx.Button(
            panel, label="補正後の連続画像とカタログファイルを作成する..."
        )
        ...
        output_file_sizer.Add(
            wx.StaticText(
                output_file_panel, label="出力した連続画像のカタログファイル:"
            ),
            flag=wx.ALIGN_CENTER_VERTICAL,
        )
        self.output_filename_text = wx.TextCtrl(
            output_file_panel, value="", style=wx.TE_READONLY
        )
        ...
        self.input_video_thumbnail.Bind(
            EVT_VIDEO_POSITION_CHANGED, self.__on_video_position_changed
        )
        self.base_frame_button.Bind(
            wx.EVT_RADIOBUTTON, self.__on_base_frame_button_clicked
        )
        self.sample_frame_button.Bind(
            wx.EVT_RADIOBUTTON, self.__on_sample_frame_button_clicked
        )
    ...
    def __make_viewer_panel(self, parent):
        ...
        caption_sizer.Add(
            wx.StaticText(caption_panel, label="ブレ補正の基準画像とブレ測定枠の設定:"),
            flag=wx.ALIGN_CENTER,
        )
        caption_sizer.Add(
            wx.StaticText(
                caption_panel, label="ブレ補正後のサンプル画像と歪み補正の設定:"
            ),
            flag=wx.ALIGN_CENTER,
        )
        caption_sizer.Add(
            wx.StaticText(
                caption_panel, label="画像補正後のサンプル画像と出力範囲の設定:"
            ),
            flag=wx.ALIGN_CENTER,
        )
        ...
        self.base_image_viewer = BaseImageViewer(
            image_panel, self.model.shaking_detection_fields, field_add_mode=True
        )
        ...
        self.deshaking_image_viewer = DeshakingImageViewer(
            image_panel,
            self.model.perspective_points,
            self.model.shaking_detection_fields,
        )
        self.deshaking_image_viewer.Bind(
            EVT_PERSPECTIVE_POINTS_CHANGED, self.__on_perspective_points_changed
        )
    ...
    def __make_console_panel(self, main_panel):
        ...
        correction_sizer.Add(
            wx.StaticText(
                correction_panel,
                label="歪み補正(射影変換)は中央の画像の赤■を動かして",
                style=wx.ST_NO_AUTORESIZE,
            ),
            flag=wx.ALIGN_CENTER_HORIZONTAL,
        )
    ...
    def __set_base_image_viewer(self):
        ...
        if (
            image_catalog is None
            or self.model.base_frame_pos is None
            or self.model.base_frame_pos >= len(image_catalog)
        ):
            self.base_image_viewer.clear()
            return
        self.base_frame = cv2.cvtColor(
            cv2.imread(
                str(image_catalog[self.model.base_frame_pos]), cv2.IMREAD_UNCHANGED
            ),
            cv2.COLOR_BGR2RGB,
        )
        self.deshaking_correction.set_base_image(self.base_frame)
        frame = (
            (self.base_frame // 256).astype(np.uint8)
            if self.base_frame.dtype == np.uint16
    ...
    def __set_sample_image_viewer(self):
        ...
        if (
            image_catalog is None
            or self.model.sample_frame_pos is None
            or self.model.sample_frame_pos >= len(image_catalog)
        ):
        ...
        # deshaking image
        self.sample_frame = cv2.cvtColor(
            cv2.imread(
                str(image_catalog[self.model.sample_frame_pos]), cv2.IMREAD_UNCHANGED
            ),
            cv2.COLOR_BGR2RGB,
        )
        if self.model.perspective_points.is_none():
            self.model.perspective_points.init(self.sample_frame)
        self.deshaking_correction.set_sample_image(
            self.sample_frame, self.model.sample_frame_pos
        )
        fields = (
            self.model.shaking_detection_fields
            if self.model.use_deshake_correction
            else []
        )
        ...
        frame = (
            (self.sample_frame // 256).astype(np.uint8)
            if self.sample_frame.dtype == np.uint16
            else self.sample_frame
        )
        deshaked_frame = cv2.warpPerspective(
            frame, mat, (frame.shape[1], frame.shape[0]), flags=cv2.INTER_AREA
        )
    ...
    def __update_model(self):
        ...
        self.model.use_deshake_correction = (
            self.use_deshake_correction_button.GetValue()
        )
        self.model.use_rotation_correction = (
            self.use_rotation_correction_button.GetValue()
        )
        self.model.use_perspective_correction = (
            self.use_perspective_correction_button.GetValue()
        )
    ...
    def __load_setting(self):
        ...
        try:
            with open(path, "r") as f:
                ...
                self.use_deshake_correction_button.SetValue(
                    self.model.use_deshake_correction
                )
                self.use_rotation_correction_button.SetValue(
                    self.model.use_rotation_correction
                )
                self.use_perspective_correction_button.SetValue(
                    self.model.use_perspective_correction
    ...
    def __on_save_button_clicked(self, event):
        if self.input_video_thumbnail.get_frame_count() == 0:
            wx.MessageBox(
                "連続画像が読み込まれていません。", "エラー", wx.OK | wx.ICON_ERROR
            )
            event.Skip()
            return
        ...
        output_filename = (
            input_path.stem.removesuffix(PNG_SUFFIX).removesuffix(TIFF_SUFFIX)
            + CORR_SUFFIX
            + ".txt"
        )
        ...
        with wx.FileDialog(
            ...
            self.output_video_thumbnail.load_image_catalog(
                input_path, self.model, output_path
            )
        event.Skip()
