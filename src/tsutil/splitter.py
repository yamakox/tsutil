...
class MainFrame(ToolFrame):
    def __init__(self, parent: wx.Window | None = None, *args, **kw):
        ...
        input_file_sizer.Add(
            wx.StaticText(input_file_panel, label='分割する連続画像のカタログファイル:'), flag=wx.ALIGN_CENTER_VERTICAL
        )
        ...
        input_video_sizer.Add(
            wx.StaticText(input_video_panel, label='赤い▲をドラッグして、分割場所のフレームを選択してください。'),
        ...
        output_file_sizer.Add(
            wx.StaticText(output_file_panel, label='結合した連続画像のカタログファイル:'), flag=wx.ALIGN_CENTER_VERTICAL
        )
