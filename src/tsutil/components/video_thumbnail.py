...
class VideoThumbnail(wx.Panel):
    ...
    def load_video(
        self, path: Path, rotation=0, filter_complex=None, output_path: Path = None, format='PNG', scale=None
    ):
    ...
    def get_frame_range(self):
        if self.loading or not self.frames:
            return 0, 0
        start = int(len(self.frames) * self.start_pos + 0.5)
        end = int(len(self.frames) * self.end_pos + 0.5)
    ...
    def __get_start_pos_x(self):
        return int(self.start_pos * self.thumbnail_size[0] + 0.5)

    def __get_end_pos_x(self):
        return int(self.end_pos * self.thumbnail_size[0] + 0.5)
    ...
    def __update_thumbnail(self):
        ...
        if thumb_width < self.frame_thumbnail_min_width:
            ...
            indices = (np.linspace(0, len(self.frames) - 1, count) + 0.5).astype(int)
        x = (np.linspace(0, self.buf.shape[1], count + 1) + 0.5).astype(int)
    ...
    def __on_paint(self, event):
        ...
        if gc:
            ...
            if self.progress_total > 0:
                ...
                gc.DrawRectangle(
                    self.RANGE_BAR_WIDTH,
                    self.thumbnail_size[1] - self.PROGRESS_BAR_HEIGHT,
                    self.thumbnail_size[0],
                    self.PROGRESS_BAR_HEIGHT,
                )
                gc.SetBrush(wx.Brush(wx.Colour(0, 255, 64, 255)))
                gc.DrawRectangle(
                    self.RANGE_BAR_WIDTH + 1,
                    self.thumbnail_size[1] - self.PROGRESS_BAR_HEIGHT + 1,
                    min(
                        self.progress_current * self.thumbnail_size[0] // self.progress_total,
                        self.thumbnail_size[0] - 2,
                    ),
            ...
            if self.use_range_bar:
                ...
                if self.__get_end_pos_x() + self.RANGE_BAR_WIDTH < self.thumbnail_size[0]:
                    gc.DrawRectangle(
                        self.RANGE_BAR_WIDTH + self.__get_end_pos_x() + self.RANGE_BAR_WIDTH,
                        0,
                        self.thumbnail_size[0] - (self.__get_end_pos_x() + self.RANGE_BAR_WIDTH),
                ...
                gc.DrawRectangle(
                    self.RANGE_BAR_WIDTH + self.__get_end_pos_x(), 0, self.RANGE_BAR_WIDTH, self.thumbnail_size[1]
                )
            ...
            if self.use_x_arrow:
                ...
                if len(self.frames) > 0 and self.frame_pos is not None:
                    x = (
                        self.RANGE_BAR_WIDTH
                        + int(self.frame_pos * (self.thumbnail_size[0] - 1) / (len(self.frames) - 1) + 0.5)
                        - self.ARROW_SIZE // 2
                    )
                else:
                ...
                gc.DrawBitmap(
                    self.bitmap_arrow_left,
                    self.RANGE_BAR_WIDTH - self.ARROW_SIZE,
                    self.thumbnail_size[1],
                    self.ARROW_SIZE,
                    self.ARROW_SIZE,
                )
                gc.DrawBitmap(
                    self.bitmap_arrow_right,
                    self.RANGE_BAR_WIDTH + self.thumbnail_size[0],
                    self.thumbnail_size[1],
                    self.ARROW_SIZE,
                    self.ARROW_SIZE,
                )
        ...
        if self.use_range_bar and y < self.thumbnail_size[1]:
            ...
            if x < self.RANGE_BAR_WIDTH:
                ...
                self.frame_pos = max(
                    0,
                    min(
                        int((len(self.frames) - 1) * (x - self.RANGE_BAR_WIDTH) / (self.thumbnail_size[0] - 1) + 0.5),
                        len(self.frames) - 1,
                    ),
                )
                self.__update_bitmap()
    ...
    def __on_mouse_move(self, event):
        ...
        if self.dragging == DRAGGING_LEFT:
            self.start_pos = (
                max(0, min(x - self.dragging_dx - self.RANGE_BAR_WIDTH, self.__get_end_pos_x()))
                / self.thumbnail_size[0]
            )
            self.__update_bitmap()
        elif self.dragging == DRAGGING_RIGHT:
            self.end_pos = (
                min(self.thumbnail_size[0], max(x - self.dragging_dx - self.RANGE_BAR_WIDTH, self.__get_start_pos_x()))
                / self.thumbnail_size[0]
            )
            self.__update_bitmap()
            ...
            self.start_pos = (
                max(0, min(x - self.dragging_dx - self.RANGE_BAR_WIDTH, self.thumbnail_size[0] - w))
                / self.thumbnail_size[0]
            )
    ...
    def __on_mouse_button_timer(self, event=None):
        if self.frames and wx.GetMouseState().LeftIsDown():
            ...
            if self.dragging == DRAGGING_LEFT_ARROW:
                ...
                frame_pos = min(self.frame_pos + 1, len(self.frames) - 1)
            if frame_pos is not None and self.frame_pos != frame_pos:
                self.frame_pos = frame_pos
                self.__update_bitmap()
                wx.QueueEvent(self, VideoPositionChangedEvent(self.frames, self.get_frame_position(), len(self.frames)))
                    ...
                    for i, frame in enumerate(reader.frames()):
                        ...
                        if output_path:
                            ...
                            future = executor.submit(
                                _save_frame, output_parent_path / image_filename, cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
                            )
                        ...
                        frame = cv2.resize(
                            frame,
                            ((w * self.thumbnail_size[1]) // h, self.thumbnail_size[1]),
                        ...
                        if failures:
                            wx.QueueEvent(
                                self, VideoLoadErrorEvent('連続画像ファイルの保存に失敗したファイルがあります。')
                            )
                        wx.QueueEvent(self, VideoLoadingEvent())
                        time.sleep(0.25)
                        self.loading = None
        ...
        try:
            ...
            if correction_model is None:
                ...
                base_frame = cv2.cvtColor(
                    cv2.imread(str(self.image_catalog[correction_model.base_frame_pos]), cv2.IMREAD_UNCHANGED),
                    cv2.COLOR_BGR2RGB,
                )
            future_list = []
            indexed_frame = {}
            ...
            def _load_and_save_frame(lock, indexed_frame, index, image_path, output, correction_model, base_frame):
                ...
                if output:
                    ...
                    if correction_model is not None:
                        ...
                        fields = (
                            correction_model.get_shaking_detection_fields(index)
                            if correction_model.use_deshake_correction
                            else []
                        )
                    ...
                    if not correction_model.clip.is_none():
                        frame = frame[
                            correction_model.clip.top : correction_model.clip.bottom,
                            correction_model.clip.left : correction_model.clip.right,
                            :,
                        ]
            ...
            with futures.ThreadPoolExecutor(max_workers=MAX_WORKERS2) as executor:
                ...
                for i, image_path in enumerate(self.image_catalog):
                    ...
                    self.progress_current = i + 1
                    future = executor.submit(
                        _load_and_save_frame, lock, indexed_frame, i, image_path, output, correction_model, base_frame
                    )
                    future_list.append(future)
                    ...
                    if failures:
                        wx.QueueEvent(
                            self, VideoLoadErrorEvent('連続画像ファイルの入出力処理に失敗したファイルがあります。')
                        )
                    self.frames = [indexed_frame[i] for i in sorted(indexed_frame.keys())]
                    wx.QueueEvent(self, VideoLoadingEvent())
                    time.sleep(0.25)
