...
class ImageViewer(wx.Panel):
    ...
    def set_image(self, image):
        ...
        if self.image is None:
            self.image_ox = image.shape[1] * 0.5
            self.image_oy = image.shape[0] * 0.5
            self.zoom_ratio = 0.0
    ...
    def get_image_position(
        self, mouse_pos: tuple[int, int] | None = None, range_limit: bool = False
    ...
    def get_image_precise_position(
        ...
        v_ox = (self.regions['preview'].GetLeft() + self.regions['preview'].GetRight()) * 0.5
        v_oy = (self.regions['preview'].GetTop() + self.regions['preview'].GetBottom()) * 0.5
    ...
    def get_view_position(self, x: float, y: float) -> tuple[int, int] | tuple[None, None]:
        ...
        v_x = (self.regions['preview'].GetLeft() + self.regions['preview'].GetRight()) * 0.5
        v_y = (self.regions['preview'].GetTop() + self.regions['preview'].GetBottom()) * 0.5
        v_x += (x - ox) * zx
        v_y += (y - oy) * zy
        return int(v_x + 0.5), int(v_y + 0.5)
    ...
    def get_view_rect(
        ...
        return int(v_x0 + 0.5), int(v_y0 + 0.5), int(v_x1 + 0.5), int(v_y1 + 0.5)
    ...
    def __update_preview(self):
        ...
        if z_y1 > self.zoomed_image.shape[0]:
            ...
            b_y1 = buf_h
        self.buf[b_y0:b_y1, b_x0:b_x1, :] = self.zoomed_image[
            z_y0 : z_y0 + (b_y1 - b_y0), z_x0 : z_x0 + (b_x1 - b_x0), :
        ]
        self.bitmap.CopyFromBuffer(self.buf.tobytes())
        self.Refresh()
    ...
    def on_size(self, event):
        ...
        self.buf = np.zeros(
            (self.client_height - self.SCROLL_BAR_SIZE, self.client_width - self.SCROLL_BAR_SIZE, 3), dtype=np.uint8
        )
        self.bitmap = wx.Bitmap.FromBuffer(
            self.client_width - self.SCROLL_BAR_SIZE, self.client_height - self.SCROLL_BAR_SIZE, self.buf.tobytes()
        )
        self.regions = {
    ...
    def on_paint(self, event, gc: wx.GraphicsContext):
        ...
        if hsl < w:
            ...
            hsl_min = max(0, int(ox - hsl * 0.5 + 0.5))
            hsl_max = min(w, int(ox + hsl * 0.5 + 0.5))
        ...
        if vsl < h:
            ...
            vsl_min = max(0, int(oy - vsl * 0.5 + 0.5))
            vsl_max = min(h, int(oy + vsl * 0.5 + 0.5))
        ...
        if self.use_grid:
            ...
            if self.grid_center_only:
                gc.DrawRectangle(rgn_ox, rgn.top, 0.5, rgn.bottom - rgn.top)
                gc.DrawRectangle(rgn.left, rgn_oy, rgn.right - rgn.left, 0.5)
            else:
                for x in range(0, (rgn.right - rgn.left) // 2, self.GRID_SIZE):
                    gc.DrawRectangle(rgn_ox + x, rgn.top, 0.5, rgn.bottom - rgn.top)
                    if x:
                        gc.DrawRectangle(rgn_ox - x, rgn.top, 0.5, rgn.bottom - rgn.top)
                for y in range(0, (rgn.bottom - rgn.top) // 2, self.GRID_SIZE):
                    gc.DrawRectangle(rgn.left, rgn_oy + y, rgn.right - rgn.left, 0.5)
                    if y:
                        gc.DrawRectangle(rgn.left, rgn_oy - y, rgn.right - rgn.left, 0.5)
            gc.ResetClip()
        if self.progress_total > 0:
            ...
            gc.DrawRectangle(
                rgn.GetLeft(), rgn.GetBottom() - self.PROGRESS_BAR_HEIGHT, rgn.GetWidth(), self.PROGRESS_BAR_HEIGHT
            )
            gc.SetBrush(wx.Brush(wx.Colour(0, 255, 64, 255)))
            gc.DrawRectangle(
                rgn.GetLeft() + 1,
                rgn.GetBottom() - self.PROGRESS_BAR_HEIGHT + 1,
                min(self.progress_current * rgn.GetWidth() // self.progress_total, rgn.GetWidth() - 2),
    ...
    def on_mouse_wheel(self, event: wx.MouseEvent):
        ...
        buf_h, buf_w = self.buf.shape[:2]
        if axis == 1:
            self.image_ox += buf_w * 0.002 * rot / self.zoom_ratio
        else:
            self.image_oy -= buf_h * 0.002 * rot / self.zoom_ratio
        self.__zoom_and_update_preview()
        self.fire_mouse_over_image(x, y)
    ...
    def on_mouse_wheel_zoom(self, event: wx.MouseEvent):
        ...
        zoom *= 1.0 + event.GetWheelRotation() * 0.001
        self.zoom_ratio = min(max(self.min_zoom_ratio, zoom), 2.0)
        self.__zoom_and_update_preview()
        self.fire_mouse_over_image(x, y)
