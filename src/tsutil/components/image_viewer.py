...
class MouseOverImageEvent(wx.Event):
...
class ImageViewer(wx.Panel):
    ...
    def get_image_position(self):
        x, y = self.__get_image_position(mouse_pos=None)
        return int(x), int(y)
    ...
    def __get_image_position(self, mouse_pos=None):
        ...
        if mouse_pos and 'preview' in self.regions:
    ...
    def __on_paint(self, event):
        ...
        if gc:
            gc.DrawBitmap(self.bitmap, 0, 0, *self.bitmap.GetSize())
    ...
    def __on_mouse_wheel(self, event):
        if self.image is None:
            return
        self.zoom_ratio *= 1.0 + event.GetWheelRotation() * .0005
        if self.zoom_ratio < self.min_zoom_ratio:
            self.zoom_ratio = self.min_zoom_ratio
