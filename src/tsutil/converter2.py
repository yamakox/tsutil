...
from .components.image_viewer import (
    ImageViewer,
    SCROLL_BAR_SIZE,
    EVT_MOUSE_OVER_IMAGE,
    EVT_MOUSE_CLICK_IMAGE,
)
...
class MainFrame(ToolFrame):
    def __init__(self, parent: wx.Window | None = None, *args, **kw):
        ...
        input_file_sizer.Add(
            wx.StaticText(
                input_file_panel, label="動画にするステッチング画像ファイル:"
            ),
        ...
        sizer.Add(
            wx.StaticText(
                panel,
                label="ステッチング画像の任意の場所をドラッグで選択して、動画で表示したい場所をリストに追加してください。",
            ),
            flag=wx.ALIGN_CENTER,
        )
        self.input_image_thumbnail = RangeImageViewer(
            panel, min_size=THUMBNAIL_SIZE, enable_zoom=True
        )
        self.input_image_thumbnail.SetCursor(wx.Cursor(wx.CURSOR_CROSS))
        sizer.Add(
            self.input_image_thumbnail, flag=wx.ALIGN_CENTER | wx.BOTTOM, border=MARGIN
        )
        row += 2
        ...
        output_file_sizer.Add(
            wx.StaticText(output_file_panel, label="調整後のステッチング画像ファイル:"),
            flag=wx.ALIGN_CENTER_VERTICAL,
        )
    ...
    def __make_setting_panel(self, parent):
        ...
        option1_sizer.Add(
            wx.StaticText(
                option1_panel, label="幅:", style=wx.ALIGN_RIGHT | wx.ST_NO_AUTORESIZE
            ),
            flag=wx.EXPAND | wx.ALIGN_CENTER_VERTICAL,
        )
        self.seq_w = wx.SpinCtrl(
            option1_panel,
            value="0",
            min=0,
            max=1000000,
        ...
        option2_sizer.Add(
            wx.StaticText(
                option2_panel, label="初速:", style=wx.ALIGN_RIGHT | wx.ST_NO_AUTORESIZE
            ),
            flag=wx.EXPAND | wx.ALIGN_CENTER_VERTICAL,
        )
        ...
        option2_sizer.Add(
            wx.StaticText(
                option2_panel, label="終速:", style=wx.ALIGN_RIGHT | wx.ST_NO_AUTORESIZE
            ),
            flag=wx.EXPAND | wx.ALIGN_CENTER_VERTICAL,
        )
        ...
        self.seq_trans_t = wx.SpinCtrlDouble(
            option2_panel,
            value="3.0",
            min=0.0,
            max=100.0,
        ...
        self.seq_still_t = wx.SpinCtrlDouble(
            option2_panel,
            value="2.0",
            min=0.0,
            max=100.0,
        ...
        self.sequence_list = wx.ListCtrl(
            setting_panel,
            size=dpi_aware_size(parent, wx.Size(*SEQUENCE_LIST_SIZE)),
            style=wx.LC_REPORT | wx.LC_SINGLE_SEL,
        )
        self.sequence_list.InsertColumn(
            0, "中心X", wx.LIST_FORMAT_RIGHT, width=dpi_aware(parent, 70)
        )
        self.sequence_list.InsertColumn(
            1, "中心Y", wx.LIST_FORMAT_RIGHT, width=dpi_aware(parent, 70)
        )
        self.sequence_list.InsertColumn(
            2, "幅", wx.LIST_FORMAT_RIGHT, width=dpi_aware(parent, 70)
        )
        self.sequence_list.InsertColumn(
            3, "初速", wx.LIST_FORMAT_CENTER, width=dpi_aware(parent, 60)
        )
        self.sequence_list.InsertColumn(
            4, "終速", wx.LIST_FORMAT_CENTER, width=dpi_aware(parent, 60)
        )
        self.sequence_list.InsertColumn(
            5, "遷移(秒)", wx.LIST_FORMAT_RIGHT, width=dpi_aware(parent, 70)
        )
        self.sequence_list.InsertColumn(
            6, "静止(秒)", wx.LIST_FORMAT_RIGHT, width=dpi_aware(parent, 70)
        )
        self.sequence_list.Bind(
            wx.EVT_LIST_ITEM_SELECTED, self.__on_seq_list_item_selected
        )
        self.sequence_list.Bind(
            wx.EVT_LIST_ITEM_DESELECTED, self.__on_seq_list_item_selected
        )
        self.sequence_list.Bind(wx.EVT_LIST_BEGIN_DRAG, self.__on_seq_list_begin_drag)
        self.sequence_list.SetDropTarget(
            ListDropTarget(self.__handle_drop, self.__handle_drag_over)
        )
    ...
    def __render_buf(self, image_x, image_y, image_w):
        ...
        if _r < self.thumb_ratio:
            ...
            src = self.raw_image
        _w = image_w / 2
        _h = int(_w * self.buf.shape[0] / self.buf.shape[1] + 0.5)
        img_x0, img_x1 = int(image_x - _w + 0.5), int(image_x + _w + 0.5)
        img_y0, img_y1 = int(image_y - _h + 0.5), int(image_y + _h + 0.5)
        buf_x0, img_x0 = (int(-img_x0 * _r + 0.5), 0) if img_x0 < 0 else (0, img_x0)
        buf_x1, img_x1 = (
            (self.buf.shape[1] - int((img_x1 - src.shape[1]) * _r + 0.5), src.shape[1])
            if img_x1 > src.shape[1]
            else (self.buf.shape[1], img_x1)
        )
        buf_y0, img_y0 = (int(-img_y0 * _r + 0.5), 0) if img_y0 < 0 else (0, img_y0)
        buf_y1, img_y1 = (
            (self.buf.shape[0] - int((img_y1 - src.shape[0]) * _r + 0.5), src.shape[0])
            if img_y1 > src.shape[0]
            else (self.buf.shape[0], img_y1)
        )
        self.buf[...] = 0
        _w2 = buf_x1 - buf_x0
        _h2 = buf_y1 - buf_y0
        self.buf[buf_y0:buf_y1, buf_x0:buf_x1, :] = cv2.resize(
            src[img_y0:img_y1, img_x0:img_x1, :],
            (_w2, _h2),
    ...
    def __movie_save_worker(self, output_path):
        ...
        total += sum(
            [(item.trans_t + item.still_t) * FPS for item in self.sequence.items[1:]]
        )
        with FrameWriter(str(output_path), size=MOVIE_SIZE, fps=FPS, qmax=16) as writer:
            try:
                counter = 1
                for seq0, seq1 in zip(
                    [None] + self.sequence.items[:-1], self.sequence.items
                ):
    ...
    def __enum_render_frame(self, seq0, seq1):
        if seq0:
            count = int(seq1.trans_t * FPS + 0.5) + 1
        ...
        for i in range(int(seq1.still_t * FPS + 0.5)):
            yield self.buf
    ...
    def __on_input_file_changed(self, event):
        ...
        self.thumb_image = cv2.resize(
            self.raw_image,
            (
                int(self.raw_image.shape[1] * self.thumb_ratio),
                dpi_aware(self, THUMBNAIL_HEIGHT),
            ),
            interpolation=cv2.INTER_AREA,
        )
        self.input_image_thumbnail.set_image(self.thumb_image)
        self.input_image_thumbnail.set_image_zoom_position(
            0, dpi_aware(self, THUMBNAIL_HEIGHT) // 2, 1.0
        )
    ...
    def __handle_drag_over(self, x, y, defResult):
        ...
        if index != self.drag_over_index:
            if self.drag_over_index != wx.NOT_FOUND:
                self.sequence_list.SetItemBackgroundColour(
                    self.drag_over_index, wx.Colour(255, 255, 255, 0)
                )
            if index != wx.NOT_FOUND:
                self.sequence_list.SetItemBackgroundColour(
                    index, wx.Colour(255, 222, 222, 255)
                )
    ...
    def __on_field_selected(self, event):
        ...
        x = int(x / self.thumb_ratio + 0.5)
        y = int(y / self.thumb_ratio + 0.5)
        w = int(w / self.thumb_ratio + 0.5)
        h = int(w * 9 / 16 + 0.5)  # w優先で16:9にする
    ...
    def __on_mouse_over_preview(self, event):
        ...
        if x is None:
            ...
            x = int((x - self.buf.shape[1] * 0.5) * r + self.raw_image_x + 0.5)
            y = int((y - self.buf.shape[0] * 0.5) * r + self.raw_image_y + 0.5)
